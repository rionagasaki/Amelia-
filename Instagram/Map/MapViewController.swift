...
class MapViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        followRef.getDocument{ (document, error) in
            ...
            if followData.follow.count > 0 {
                ...
                let Ref = Firestore.firestore().collection(Const.PostPath).whereField("uid", in: followData.follow)
                print("ref\(Ref)")
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        ...
        let alert = UIAlertController(title: "友達が投稿しています。", message: "ぜひ向かってみよう！", preferredStyle: .actionSheet)
